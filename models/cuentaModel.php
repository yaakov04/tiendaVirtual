...
class cuentaModel{
    ...
    public function actualizarPasswordDB($new_password){
        
        $id_usuario = $_SESSION['id'];
        $opciones=array(
            'cost' => 12
        );
        $password= password_hash($new_password, PASSWORD_BCRYPT, $opciones);

        try{
            require 'config/conexion_bd.php';
            $stmt = $conexion->prepare(" UPDATE usuarios SET  password = ?, editado = NOW() WHERE id = ? ");
